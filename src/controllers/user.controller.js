...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    if (!coverImage.url) {
        throw new ApiError(400, "Error while uploading cover image at cloudinary")
    }
...
// error  showing nothing
const updateUserAvatar = asyncHandler(async (req, res) => {
    const avatarLocalPath = req.file?.path
    const oldAvatar = req.user?.avatar
    if(!oldAvatar){
        throw new ApiError(401, "Old Avatar is missing")
    }
    ...
    if (!avatar.url) {
        throw new ApiError(400, "Error while uploading avatar at cloudinary")
    }
